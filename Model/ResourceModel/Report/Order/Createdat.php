 ...
 */
namespace Excellence\Gst\Model\ResourceModel\Report\Order;
    ...
    protected function _aggregateByField($aggregationField, $from, $to)
    {
        ...
        try {
            ...
            $columns = [
                ...
                'cgst'            => new \Zend_Db_Expr('SUM(ot.cgst)'),
                'sgst'            => new \Zend_Db_Expr('SUM(ot.sgst)'),
                'igst'            => new \Zend_Db_Expr('SUM(ot.igst)'),
            ];
            ...
            $select->from(
                ...
                []
            )->join(array('ot' => $this->getTable('sales_order_tax')), 'ot.order_id = o.entity_id', array())
            ->where(
                'o.state NOT IN (?)',
