 ...
 */
namespace Excellence\Gst\Block\Adminhtml\Order\View\Items\Renderer;
...
{
    /**
     * @return \Magento\Sales\Block\Adminhtml\Items\Renderer\DefaultRenderer
     */
    protected function _prepareLayout()
    {
        $this->setTemplate("Excellence_Gst::order/view/items/renderer/default.phtml");
    ...
    public function getColumnHtml(\Magento\Framework\DataObject $item, $column, $field = null)
    {
        ...
        switch ($column) {
            ...
            case 'tax-amount':
                ...
                break;
            case 'hsn':
                $productData = $this->_productFactory->load($item->getProductId());
                $html = ($productData->getHsn()) ? $productData->getHsn() : "";
                break;
            case 'sgst':
                $originalTaxAmount = $item->getTaxAmount();
                $taxAmount = $originalTaxAmount / 2;
                $item->setTaxAmount($taxAmount);
                $html = $this->displayPriceAttribute('tax_amount');
                $item->setTaxAmount($originalTaxAmount);
                break;
            case 'tax-percent':
                $html = $this->displayTaxPercent($item);
                break;
            case 'state-tax-percent':
                $originalTaxPercent = $item->getTaxPercent();
                $taxPercent = $originalTaxPercent / 2;
                $item->setTaxPercent($taxPercent);
                $html = $this->displayTaxPercent($item);
                $item->setTaxPercent($originalTaxPercent);
                break;
            case 'discont':
